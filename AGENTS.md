...
- **orchestrator** — `sip` runs the suite's clean-and-true checks; `re0-loop` and `nba` run and navigate the coil cycle. Above that loop, `re0-supervisor` selects one gate and routes evidence, `re0-worker` executes the gate through `re0-loop`'s discipline, and `re0-reviewer` independently applies `sip` before completion. They use separate host contexts when available and the same artifact-mediated sequence when not; native agents are an optimization, never a package dependency. `re0-release` runs the shipping and releasing checklist, runs `sip` when installed, and applies commit-economy directly. It does not auto-fire user-invoked `re0-git`; if an existing commit needs cleanup, it asks the human to run `re0-git`. `re0-plan` opens the iteration folder `re0-release` later retires. `re0-merge` walks reviewing and landing a contribution, calling `shower`, `re0-git`, and `re0-release` when present. An orchestrator degrades gracefully: it uses the model-invoked skills present and skips the rest. It recommends rather than auto-fires user-invoked judgment skills such as `hate`, `macrothink`, and `prism`.
...
`re0-supervisor` adds `SUPERVISOR.local.md` to that casebook and saves worker/reviewer reports as local evidence. It does not make `.re0/` canonical: requirements remain in project requirements and design docs, proof remains in EVIDENCE, and the state file stores only routing facts needed to resume.

## Installation compatibility

The published package remains plain Markdown with no runtime dependencies. The repository's `install.sh` is the Node-free core path for Claude Code and Codex; it uses POSIX shell/file operations, diagnoses optional runtimes and browsers, and only replaces paperthin-managed entries unless the user explicitly requests a backed-up overwrite. `npx skills add` remains the cross-agent managed-distribution path and necessarily inherits the selected `skills` CLI's Node/npm and platform requirements. Never present an optional native-agent, hook, Playwright, Chromium, Homebrew, Node, or npm failure as a core catalog failure.
